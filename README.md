...
UI application for matching the NOMIS identifier of a prisoner to a learning record held by DFE's Learning Records Service (LRS). This project is based on the HMPPS Typescript project template
...
* `prisoner-search` - Used to return a list of prisoners which relate to the serch term provided on the 'find a prisoner' page. `PRISONER_SEARCH_API_URL` env variable sets the url.
